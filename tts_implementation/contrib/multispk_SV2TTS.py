...
from tts_implementation.SV2TTS.decoder import Decoder
from tts_implementation.SV2TTS.encoder import Encoder
from tts_implementation.SV2TTS.postnet import Postnet
...
class MultiSpkTacotron2(nn.Module):
    ...
    def __init__(
        ...
        #spk_emb_dim=64,
        spk_emb_dim=512,
        spk_output_dimension=64,
    ):
        super().__init__()
        # speaker embeddingを定義
        ...
        self.encoder = Encoder(
            ...
            encoder_dropout,
            spk_emb_dim,
            spk_output_dimension,
        )
        self.decoder = Decoder(
            encoder_hidden_dim + spk_output_dimension,
            decoder_out_dim,
            decoder_layers,
            decoder_hidden_dim,
    ...
    def forward(self, seq, in_lens, decoder_targets, spk_ids):
        ...
        encoder_outs, spk_emb =  self.encoder(seq, in_lens, spk_ids)

        # 話者埋め込み
        #spk_emb = self.spk_embed(spk_ids)
        spk_batch_size = seq.size()[0]
        spk_num_chars = seq.size()[1]
        if spk_emb.dim() == 1:
            idx = 0
        else:
            idx = 1
        spk_emb_size = spk_emb.size()[idx]
        spk_emb = spk_emb.repeat_interleave(spk_num_chars, dim=idx)
        spk_emb = spk_emb.reshape(spk_batch_size, spk_emb_size, spk_num_chars)
        ...
        decoder_inp = torch.cat((encoder_outs, spk_emb), 2)
        #decoder_inp = self.encoder(seq, in_lens, spk_ids)
    ...
    def inference(self, seq, spk_id):
        ...
        Args:
            seq (torch.Tensor): input sequence
            spk_id (int): speaker id numpy arrray wav
        Returns:
            tuple: tuple of outputs, outputs (after post-net), stop token prediction
                and attention weights.
        """
        ...
        outs, outs_fine, logits, att_ws = self.forward(seq, in_lens, None, spk_id.float())
