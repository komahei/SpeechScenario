...
from tts_implementation.logger import getLogger
from tts_implementation.train_util import (
    ensure_divisible_by,
    num_trainable_params,
    set_epochs_based_on_max_steps_,
)
from tts_implementation.util import init_seed, load_utt_list, pad_1d, pad_2d
...
def collate_fn_ms_tacotron(batch, reduction_factor=1):
    ...
    # 追加
    spk_lens = [len(spk_id) for spk_id in spk_ids]
    in_max_len = max(in_lens)
    out_max_len = max(out_lens)
    # 追加
    spk_max_len = max(spk_lens)
    ...
    # 追加
    spk_batch = torch.stack([torch.from_numpy(pad_1d(spk_id, spk_max_len)) for spk_id in spk_ids])
    ...
    return x_batch, il_batch, y_batch, ol_batch, stop_flags, spk_batch
