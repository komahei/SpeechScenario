...
class Encoder(nn.Module):
    ...
    def __init__(
        ...
        self.spk_dense = nn.Linear(spk_emb_dim, spk_output_dimension)

        # initialize
        self.apply(encoder_init)

    def forward(self, seqs, in_lens, spk_ids):
        """Forward step
        Args:
        ...
        spk_emb = self.spk_dense(spk_ids)
        """
        spk_batch_size = seqs.size()[0]
        spk_num_chars = seqs.size()[1]
        if spk_embeddings.dim() == 1:
            idx = 0
        else:
            idx = 1
        spk_emb_size = spk_embeddings.size()[idx]
        spk_emb = spk_embeddings.repeat_interleave(spk_num_chars, dim=idx)
        spk_emb = spk_embeddings.reshape(spk_batch_size, spk_emb_size, spk_num_chars)
        spk_emb = spk_embeddings.transpose(1, 2)
        """
        #print("ENCODERはここ!!!!!!!!!!!!!!!!!!!!!!!!",out.shape) # 32, 150, 512
        #print("SPEAKER ENCODERはここ!!!!!!!!!!!!!!!!!",spk_emb.shape) # 32, 64
        #out = torch.cat((out, spk_emb), 2)
